...
/// Should finite fields limbs be stored in little endian or big endian order?
///
/// Note that this decision _only_ affects the order in which digits are stored in the limbs array.
/// It does _not_ cause the bit representation of the underlying `u64` digits to differ from the
/// host architecture's endianness.
        ...
        impl ::ff::Field for #name {
            ...
            fn random(mut rng: impl ::ff::derive::rand_core::Rng) -> Self {
                loop {
                    ...
                    tmp.0.as_mut_slice()[#top_limb_index] &= 0xffffffffffffffffu64.checked_shr(REPR_SHAVE_BITS).unwrap_or(0);
...
pub fn num_u64_limbs_needed(modulus: &BigUint) -> usize {
    let mut limbs = 1;
    {
...
/// Generate the impls needed for a `#[derive(PrimeField)]` invocation.
pub fn prime_field_impls(
    ident: &syn::Ident,
    modulus: &BigUint,
    generator: &BigUint,
    endianness: ReprEndianness,
    ...
    let repr_ident = syn::Ident::new(&format!("{}Repr", ident), proc_macro2::Span::call_site());
    let limbs = num_u64_limbs_needed(modulus);

    let mut gen = proc_macro2::TokenStream::new();
    let (constants_impl, sqrt_impl) =
        prime_field_constants_and_sqrt(ident, modulus, limbs, generator);
...
/// Options to configure code generation of a prime field struct.
///
/// Unlike [`prime_field_impls`], `PrimeFieldCodegen` generates the code for _both_ the struct
/// definition, as well as the required impls.
///
/// It should be used like so:
/// ```rust
/// println!("{}", ff_codegen::PrimeFieldCodegen {
///     ident: "Fp",
///     is_pub: false,
...
pub struct PrimeFieldCodegen<'a> {
    /// The name of the struct to generate
    pub ident: &'a str,
    /// If `true`, then the generated struct will be marked as `pub`.
    pub is_pub: bool,
    /// The modulus of the field, written as a decimal string
    pub modulus: &'a str,
    /// A generator of the field, written as a decimal string
    pub generator: &'a str,
    pub endianness: ReprEndianness,
}
impl std::fmt::Display for PrimeFieldCodegen<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let ident = syn::Ident::new(&self.ident, proc_macro2::Span::call_site());
        let modulus = BigUint::from_str(self.modulus).unwrap();
        let generator = BigUint::from_str(self.generator).unwrap();
        let limbs = num_u64_limbs_needed(&modulus);
        let mut gen = proc_macro2::TokenStream::new();
        if self.is_pub {
            gen.extend(quote! {
                #[repr(transparent)]
                pub struct #ident([u64; #limbs]);
            });
        } else {
            gen.extend(quote! {
                #[repr(transparent)]
                struct #ident([u64; #limbs]);
            });
        }
        gen.extend(prime_field_impls(
            &ident,
            &modulus,
            &generator,
            self.endianness,
